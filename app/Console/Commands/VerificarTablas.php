...
class VerificarTablas extends Command
{
    ...
    public static function EXCEPTION_TABLE(){ $table = ['tbl_conexion','tbl_consulta','tbl_comando','tbl_correo','tbl_plano','tbl_formato','tbl_plano_funcion','tbl_log','tbl_quemado']; return $table; }
    ...
    public static function TABLE_SOAP(){
        $dataConexion = Conexion::where('estado',1)->first(); $listaConsulta = Consulta::where('prioridad',1)->first(); $sentencia = Funciones::ParametroSentencia($listaConsulta,$dataConexion); $xml = Funciones::consultaStructuraXML($dataConexion->conexion,$dataConexion->cia,$dataConexion->proveedor,$dataConexion->usuario,$dataConexion->clave,$sentencia,$dataConexion->consulta,1,0);
        $resultado = Funciones::SOAP($dataConexion->url, $xml);
        return $resultado;
    }
